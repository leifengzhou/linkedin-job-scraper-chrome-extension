...
test("getJobCards returns dismissible componentkey role-button cards", () => {
  ...
  assert.equal(getCardKey(jobCard), "card-1");
});
...
test("extractApplyAction reads Easy Apply or company-website CTA from the detail header", () => {
  const easyApplyButton = createElement({
    innerText: "Easy Apply",
    attributes: { "aria-label": "Easy Apply to Senior Product Manager" }
  });
  ...
  const detailRoot = createElement({
    query: {
      'button[aria-label*="Easy Apply"]': easyApplyButton,
      'a[aria-label="Apply on company website"]': companyWebsiteLink
    }
  });
  ...
  assert.deepEqual(extractApplyAction(detailRoot), {
    applyType: "Easy Apply",
    href: "",
    ariaLabel: "Easy Apply to Senior Product Manager",
    isEasyApply: true
  });
